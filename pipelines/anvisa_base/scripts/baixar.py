...
logging.info(
    f"Período de coleta: {cfg.MES_INICIO:02d}/{cfg.ANO_INICIO} até {cfg.MES_FIM:02d}/{cfg.ANO_FIM}"
)
...
def scrape_anvisa_links(html_content: str | bytes | None = None):
    """Raspa a página da Anvisa (ou HTML local) para encontrar os links dos arquivos."""
    if html_content is None:
        logging.info(f"Acessando {cfg.URL_ANVISA} para extrair links...")
        html_content = requests.get(cfg.URL_ANVISA, timeout=30).content
    else:
        logging.info(f"Utilizando HTML local pré-processado para extração de links {cfg.TIPO_LISTA}...")
    ...
    href_tokens_por_tipo = {
        "PMC": ("xls_conformidade_site",),
        "PMVG": ("xls_conformidade_gov",),
    }
    conformidade_tokens = set(tok for tokens in href_tokens_por_tipo.values() for tok in tokens)
    conformidade_tokens.add("xls_conformidade_portal")

    def detect_secao_tipo(texto: str):
        if not texto:
            return None
        texto_norm = unicodedata.normalize("NFKD", texto).encode("ascii", "ignore").decode("ascii").lower()
        if "preco maximo ao consumidor" in texto_norm or "preco fabrica" in texto_norm:
            return "PMC"
        if "compras publicas" in texto_norm or "pmvg" in texto_norm:
            return "PMVG"
        return None
    ...
    if core is None:
        logging.debug("div#content-core não encontrada; usando raiz do HTML recebido.")
        core = soup
    ...
    for node in core.descendants:
        if isinstance(node, Tag) and node.name in {"h2", "h3", "h4", "h5", "strong"}:
            novo_tipo = detect_secao_tipo(node.get_text(" ", strip=True))
            if novo_tipo:
                ctx_tipo = novo_tipo
...
def download_files(df_to_download):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=cfg.MAX_DOWNLOAD_WORKERS) as exe:
...
def clean_downloaded_files(source_folder, target_folder):
    ...
    logging.info(f"Iniciando limpeza de {len(all_files)} arquivos com {cfg.MAX_CLEANING_THREADS} threads...")
    with concurrent.futures.ThreadPoolExecutor(max_workers=cfg.MAX_CLEANING_THREADS) as exe:
...
def consolidate_cleaned_files(source_folder, output_file):
    ...
    for file in tqdm(csv_files, desc="Lendo CSVs limpos", ncols=100):
        try:
            ...
            colunas_pmc = [c for c in df.columns if c.upper().startswith('PMC')]
            colunas_final = colunas_existentes + [c for c in colunas_pmc if c not in colunas_existentes]
            dfs.append(df[colunas_final])
        except Exception as e:
...
def process_vigencias(df_consolidado):
    ...
    expected_cols = ['PF 0%', 'PF 20%', 'PMVG 0%', 'PMVG 20%', 'ICMS 0%', 'CAP']
...
def main():
    ...
    if os.path.exists(cfg.PASTA_DOWNLOADS_BRUTOS):
        shutil.rmtree(cfg.PASTA_DOWNLOADS_BRUTOS)
        logging.info(f"Pasta antiga '{cfg.PASTA_DOWNLOADS_BRUTOS}' removida.")
    
    # Criar estrutura de pastas necessárias (sem apagar arquivos de outras pipelines)
    os.makedirs(cfg.PASTA_DOWNLOADS_BRUTOS, exist_ok=True)
    os.makedirs(os.path.join(cfg.PASTA_ARQUIVOS_LIMPOS, "anvisa"), exist_ok=True)
    ...
    if cfg.USE_LOCAL_HTML_SNIPPETS:
        ...
        if snippet_path:
            ...
            if path_obj.is_dir():
                # Carregar todos os HTMLs do diretório
                html_parts = []
                for f in sorted(path_obj.glob("*.html")):
                    try:
                        html_parts.append(f.read_text(encoding="utf-8"))
                        logging.info(f"Snippet carregado: {f.name}")
                    except Exception as e:
                        logging.warning(f"Erro ao ler snippet {f}: {e}")
                if html_parts:
                    html_override = "\n".join(html_parts)
                    logging.info(f"Total de {len(html_parts)} snippets carregados para {cfg.TIPO_LISTA}.")
                else:
                    logging.warning(f"Nenhum arquivo .html encontrado em {snippet_path}.")
            elif path_obj.exists():
                html_override = path_obj.read_text(encoding="utf-8")
                logging.info(f"HTML local carregado de {snippet_path} (modo teste {cfg.TIPO_LISTA}).")
            else:
                logging.warning(f"HTML local configurado não encontrado em {snippet_path}. Usando página oficial.")

    try:
        df_links = scrape_anvisa_links(html_override)
    except Exception as e:
    ...
    clean_downloaded_files(cfg.PASTA_DOWNLOADS_BRUTOS, cfg.PASTA_ARQUIVOS_LIMPOS)
    df_consolidado = consolidate_cleaned_files(cfg.PASTA_ARQUIVOS_LIMPOS, cfg.ARQUIVO_CONSOLIDADO_TEMP)
    ...
    df_vigencias_final.to_csv(cfg.ARQUIVO_FINAL_VIGENCIAS, sep=';', index=False)
    logging.info(f"[OK] Pipeline concluido! Arquivo final salvo em: {os.path.abspath(cfg.ARQUIVO_FINAL_VIGENCIAS)}")
    ...
    if os.path.exists(output_copy_path):
        ...
        time_source = os.path.getmtime(cfg.ARQUIVO_FINAL_VIGENCIAS)
        deve_copiar = time_source > time_output
    
    if deve_copiar:
        import shutil as sh
        sh.copy2(cfg.ARQUIVO_FINAL_VIGENCIAS, output_copy_path)
